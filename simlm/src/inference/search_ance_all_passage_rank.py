...
def _get_topk_result_save_path(worker_idx: int) -> str:
    # 수정: args.search_topk 값에 따라 파일 이름에 'all'로 표시
    topk_str = "all"
    return '{}/top{}_{}_{}.txt'.format(args.search_out_dir, topk_str, args.search_split, worker_idx)
...
def _worker_encode_queries(gpu_idx: int) -> Tuple:
    ...
    tokenizer: PreTrainedTokenizerFast = AutoTokenizer.from_pretrained('sentence-transformers/msmarco-roberta-base-ance-firstp')
    ...
    for batch_dict in tqdm.tqdm(data_loader, desc='query encoding', mininterval=5):
        ...
        with torch.cuda.amp.autocast() if args.fp16 else nullcontext():
            outputs = model(batch_dict)['sentence_embedding']
        encoded_embeds.append(outputs.cpu())
...
def _worker_batch_search(gpu_idx: int):
    ...
    for shard_idx, shard_path in enumerate(embeds_path_list):
        ...
        # 배치 크기 단위로 query와 특정 shard의 passage 임베딩 계산
        for start in tqdm.tqdm(range(0, len(query_ids), args.search_batch_size),
            ...
            # query embedding과 passage embedding의 내적 계산 -> [Batch, Passages]
            batch_score = torch.mm(batch_query_embed, shard_psg_embed.t())
            # 수정: args.search_topk가 -1이면 전체 passage에 대해 내림차순 정렬, 그렇지 않으면 상위 k개 선택
            ...
            for batch_idx, query_id in enumerate(batch_query_ids):
                # 배치의 정렬된 passage 인덱스를 리스트로 변환 (전체 또는 상위 k개)
                cur_indices = [idx + psg_idx_offset for idx in batch_sorted_indices[batch_idx].cpu().tolist()]
                # 현재 쿼리 ID에 대해 (score, doc_id) 튜플을 추가하여 정렬하지만, 최종 저장 시 score는 사용하지 않음
                # (전체 ranking인 경우 슬라이싱 없이 모두 추가)
                query_id_to_topk[query_id] += list(zip(batch_sorted_score[batch_idx].cpu().tolist(), cur_indices))
    ...
    with open(out_path, 'w', encoding='utf-8') as writer:
        # 수정: score 정보 없이 query_id, doc_id, rank만 저장
        for query_id in query_id_to_text:
            for rank, (_score, doc_id) in enumerate(query_id_to_topk[query_id]):
                writer.write('{}\t{}\t{}\n'.format(query_id, doc_id, rank + 1))

    logger.info('Write ranking info to {} done'.format(out_path))
