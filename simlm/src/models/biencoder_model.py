...
class BiencoderModel(nn.Module):
    ...
    def _encode(self, encoder: PreTrainedModel, input_dict: dict) -> Optional[torch.Tensor]:
        if self.args.mean_pooling: 
            if not input_dict:
                return None
            # kd_labels 제외한 나머지 입력을 encoder에 전달하여 출력 얻기
            outputs = encoder(**{k: v for k, v in input_dict.items() if k not in ['kd_labels']}, return_dict=True)
            hidden_state = outputs.last_hidden_state  # (batch, seq_length, hidden_size)

            # attention_mask를 가져옵니다. (shape: [batch, seq_length])
            attention_mask = input_dict.get("attention_mask")
            if attention_mask is None:
                raise ValueError("Mean pooling을 위해 attention_mask가 필요합니다.")

            # mean pooling 적용: attention_mask가 0인 토큰의 임베딩은 0으로 마스킹한 후, 전체 토큰 임베딩의 합을 
            # 각 배치의 유효 토큰 수로 나눕니다.
            masked_hidden_state = hidden_state.masked_fill(~attention_mask[..., None].bool(), 0.0)
            pooled = masked_hidden_state.sum(dim=1) / attention_mask.sum(dim=1, keepdim=True)

            # l2_normalize 옵션이 켜져 있다면 정규화 적용
            if self.args.l2_normalize:
